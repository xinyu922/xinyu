...
//     ROS_INFO ("\tMarker Pattern Filename: %s", pattern_filename_);
    ...
    uvPub_ = n_.advertise<ar_pose::uv>("uv", 10);
...
// DEPRECATED: Fuerte support ends when Hydro is released
  ...
  void ARSinglePublisher::getTransformationCallback (const sensor_msgs::ImageConstPtr & image_msg)
  {
    ...
    if (k != -1)
    {
      ...
      ROS_INFO ("   u: %3.5f  ;;  v: %3.5f  ;;", marker_info[k].pos[0],marker_info[k].pos[1]);
      uv_msg.header.frame_id = "uv_frame";
      uv_msg.header.stamp = ros::Time::now();
      uv_msg.u = marker_info[k].pos[0];
      uv_msg.v = marker_info[k].pos[1];
      uvPub_.publish(uv_msg);
      //
...
// DEPRECATED: Fuerte support ends when Hydro is released
        ...
        tf::poseTFToMsg(markerPose, rvizMarker_.pose);
			  ...
			  rvizMarker_.id = 1;
                          ...
                          rvizMarker_.scale.x = 5.0 * markerWidth_ * AR_TO_ROS;
                          rvizMarker_.scale.y = 5.0 * markerWidth_ * AR_TO_ROS;
                          rvizMarker_.scale.z = 2.5 * markerWidth_ * AR_TO_ROS;
