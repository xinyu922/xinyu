...
minRMS=100;
%%  改变加载文件的路径
sensor_path='wurenji\20191113\5\aft_mapped_to_init.txt'; 
ture_path='wurenji\20191113\5\20191113-005.txt';
%%
...
for take_off_stamp = -take_off_stamp1-1.8:0.1:-take_off_stamp1+2.5
    ...
    for i=1:size(cell_es,1)
        ...
        %yy=strsplit(xx,{',','-'});%如果加入-的分割符号，那么负号也会被分开,而不仅仅分开的是日期的 -
        yy=strsplit(xx,{','});
    ...
    P_es = est(:,2:4)';
    %% alignment
    [Ids_es2, Ids_gt2] = findIds (time_es, time_gt, 0.055);  %0.0035是可以保证既没有多解也没有少很多数据的值
	...
	if RMSE_trans < 0.1
		...
		title('全站仪真值与多源传感器融合导航结果对比','FontSize',15);
		figure;
		subplot(311);
		plot(errX);
