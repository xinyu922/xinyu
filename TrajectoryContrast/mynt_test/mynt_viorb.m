...
G=importdata ('KeyFrameNavStateTrajectory.txt');    % provided by EuRoC
aft_mappep_to_init=G(:,1:4);

%%

cell_aft_mappep_t=G(1:size(G),1:1);
aft_mapped_t=zeros(size(cell_aft_mappep_t,1),1);
for i=1:size(cell_aft_mappep_t,1)
    aft_mapped_t(i,1)=cell_aft_mappep_t(i,1)/1e9;
end
%先把大致的坐标移动到原点,因为这里sim3DataAlignment是一个七参数模型,会估计尺度，不先平移坐标尺度变化会很大
GNGGA(:,1)=aft_mappep_to_init(:,2);
GNGGA(:,2)=aft_mappep_to_init(:,3);
GNGGA(:,3)=aft_mappep_to_init(:,4);
...
[Ids_gt2,Ids_es2 ] = findIds (time_gt, time_es, 0.035);
...
axis equal;
grid on;
lgd = legend('RTS','mynt-maplab');
set(lgd,'Fontname','Times New Roman','FontWeight','bold','FontSize',15);
title('RTS真值与mynt-maplab结果对比','FontSize',15);
xlabel('x轴,m')
ylabel('y轴,m')
zlabel('z轴,m')
