...
filename_husky = 'turtlebot3_poses.csv'
...
index_ls = ['p1-2','p2-3','p3-4','p4-5','p5-6', 'p6-1']

#获取每个点在loop中的平均值
df_1 = erro
data_test = []
n_points = 6
for i in range(0, len(df_1), n_points):
    df_temp_1 = df_1.time[i:i+n_points]
#     print(df_temp_1)
    df_temp_2 = df_1.dist[i:i+n_points]
#     print(df_temp_2)
    df_temp_3 = df_1.vel[i:i+n_points]
...
loop_mean_time = list_mean[0:6]
loop_mean_dist = list_mean[6:12]
loop_mean_vel = list_mean[12:]
...
index_list_out = ['mean','p1-2','p2-3','p3-4','p4-5','p5-6', 'p6-1']
...
real_dis = real_dis.diff(1).shift(-1)[0:6]
...
data = {'point':[1,2,3,4,5,6]}  #创建新的dataframe
df_robot = pd.DataFrame(data)

#获取5个loop的时间信息
df_robot['time_loop_1'] = data_robot.time[0:6].reset_index(drop = True)
df_robot['time_loop_2'] = data_robot.time[6:12].reset_index(drop = True)
df_robot['time_loop_3'] = data_robot.time[12:18].reset_index(drop = True)

#获取5个loop的距离信息
df_robot['dist_loop_1'] = data_robot.dist[0:6].reset_index(drop = True)
df_robot['dist_loop_2'] = data_robot.dist[6:12].reset_index(drop = True)
df_robot['dist_loop_3'] = data_robot.dist[12:18].reset_index(drop = True)


#获取5个loop的速度信息
df_robot['vel_loop_1'] = data_robot.vel[0:6].reset_index(drop = True)
df_robot['vel_loop_2'] = data_robot.vel[6:12].reset_index(drop = True)
df_robot['vel_loop_3'] = data_robot.vel[12:18].reset_index(drop = True)
...
plt.ylabel('time/s')
plt.legend(loc = 'best')
plt.title('waypoints time for 3 loops')
...
plt.legend(loc = 'best')
plt.title('waypoints distance for 3 loops')
...
plt.ylabel('dist / dis')
plt.legend(loc = 'best')
plt.title('waypoints distance/dis for 3 loops')
...
plt.legend(loc = 'best')
plt.title('waypoints velocity for 3 loops')
